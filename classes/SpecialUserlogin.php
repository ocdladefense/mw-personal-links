...
class LoginForm extends SpecialPage {
	...
	public function authenticateUserData() {
		...
		$this->mExtUser = ExternalUser::newFromName( $this->mUsername );
			ttail('evaluating new user.','api');
		# TODO: Allow some magic here for invalid external names, e.g., let the
		# user choose a different wiki name.
		$u = User::newFromName( $this->mUsername );
					ttail('username is: '.$this->mUsername,'api');
		if( !( $u instanceof User ) || !User::isUsableName( $u->getName() ) ) {
			ttail('new user not valid.','api');
			return self::ILLEGAL;
		}

		$isAutoCreated = false;
						ttail('autocreate prep.'.$u->getID(),'api');
		if ( 0 == $u->getID() ) {
			$status = $this->attemptAutoCreate( $u );
			ttail('autocreate status is: ',$status);
